...
void C_givens(double a, double b, double *c, double *s) {
  if (b==0) {
    *c = 1;
    *s = 0;
  }
...
void C_rowrot(double *A, int i1, int i2, int m, int n, int j1, int j2, double c, double s) {
  int j;
  double t1,t2;
  for (j=j1; j<=j2; j++) {
    t1 = A[i1+j*m];
...
void C_colrot(double *A, int j1, int j2, int m, int n, int i1, int i2, double c, double s) {
  int i;
  double t1,t2;
  for (i=i1; i<=i2; i++) {
    t1 = A[i+j1*m];
...
void C_downdate1(double *Q1, double *R, int *j0p, int *mp, int *np, int *column_indices) {
  ...
  for (j=j0+1; j<n; j++) {
    ...
    C_givens(R[j-1+j*n],R[j+j*n],&c,&s);

    // Pre-multiply R
    C_rowrot(R,j-1,j,n,n,j,n-1,c,s);
...
void C_update1(double *Q2, double *w, int *mp, int *kp) {
  int m,k,j;
  m = *mp;
  k = *kp;
  ...
  for (j=k-1; j>=1; j--) {
    // Compute the appropriate c and s
    C_givens(w[j-1],w[j],&c,&s);

    // Pre-multiply w
    C_rowrot(w,j-1,j,k,1,0,0,c,s);

    // Post-multiply Q2
    C_colrot(Q2,j-1,j,m,k,0,m-1,c,s);
  }
}

// Downdate the QR factorization after deleting the first row,
// where Q is m x m and R is m x n
void C_downdate2(double *Q, double *R, int *mp, int *np) {
  int m,n,i;
  m = *mp;
  n = *np;
  ...
  for (i=m-1; i>=1; i--) {
    // Compute the appropriate c and s
    C_givens(Q[(i-1)*m],Q[i*m],&c,&s);

    // Post-mutiply Q
    C_colrot(Q,i-1,i,m,m,0,m-1,c,s);

    // Pre-multiply R
    if (i<=n) C_rowrot(R,i-1,i,m,n,i-1,n-1,c,s);
  }
}
...
void C_update2(double *y, double *D, double *r, int *mp, int *np, int *qp) {
  int m,n,q,j;
  m = *mp;
  n = *np;
  q = *qp;
  ...
  for (j=0; j<q-1; j++) {
    // Compute the appropriate c and s
    C_givens(r[j+1],r[j],&c,&s);

    // Pre-multiply r
    C_rowrot(r,j+1,j,n,1,0,0,c,s);

    // Post-mutiply D
    C_colrot(D,j+1,j,m,n,0,m-1,c,s);

    // Pre-multiply y
    C_rowrot(y,j+1,j,n,1,0,0,c,s);
  }
}

// Make the R factor upper triangular, by Givens rotating its
// columns. Here A is m x n, and R is n x n with rank(R) = n-k
void C_maketri1(double *y, double *A, double *R, int *mp, int *np, int *kp) {
  int m,n,k,i,j;
  m = *mp;
  n = *np;
  k = *kp;
  ...
  for (i=n-k-1; i>=0; i--) {
    for (j=i; j<i+k; j++) {
      // Compute the appropriate c and s
      C_givens(R[i+(j+1)*n],R[i+j*n],&c,&s);

      // Post-multiply R
      C_colrot(R,j+1,j,n,n,0,i,c,s);

      // Post-multiply D
      C_colrot(A,j+1,j,m,n,0,m-1,c,s);
...
void C_maketri2(double *y, double *A, double *R, int *mp, int *np, int *kp) {
  int m,n,k,r,d,i,j;
  m = *mp;
  n = *np;
  k = *kp;
  ...
  for (i=r-k-1; i>=0; i--) {
    for (j=i; j<i+k+d; j++) {
      // Compute the appropriate c and s
      C_givens(R[i+(j+1)*m],R[i+j*m],&c,&s);

      // Post-multiply R
      C_colrot(R,j+1,j,m,n,0,i,c,s);
...
void C_maketri3(double *y, double *A, double *R, int *m1p, int *m2p, int *np, int *qp) {
  int m1,m2,n,q,i;
  m1 = *m1p;
  m2 = *m2p;
  n = *np;
  ...
  for (i=n-q-2; i>=0; i--) {
    // Compute the appropriate c and s
    C_givens(R[i+(i+q+1)*m2],R[i+(i+q)*m2],&c,&s);

    // Post-multiply R
    C_colrot(R,i+q+1,i+q,m2,n,0,i,c,s);

    // Post-multiply D
    C_colrot(A,i+q+1,i+q,m1,n,0,m1-1,c,s);

    // Pre-multiply y
    C_rowrot(y,i+q+1,i+q,n,1,0,0,c,s);
  }
}
...
void C_maketri4(double *y, double *A, double *Q, double *R, int *m1p, int *m2p, int *np, int *qp, int *kp) {
  int m1,m2,n,q,k,i,j;
  m1 = *m1p;
  m2 = *m2p;
  n = *np;
  ...
  for (j=k+q+1; j<n; j++) {
    // Compute the appropriate c and s
    C_givens(R[j-q-1+j*m2],R[j-q+j*m2],&c,&s);

    // Pre-multiply R
    C_rowrot(R,j-q-1,j-q,m2,n,j,n-1,c,s);

    // Post-multiply Q
    C_colrot(Q,j-q-1,j-q,m2,m2,0,m2-1,c,s);
  }
}
